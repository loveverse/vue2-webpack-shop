          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  ...
                  @click="addActive(item.spuSaleAttrValueList, saleAttrValue)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <!-- 不能使用全等，点击按钮时v-model值是number，输入框输入时是string -->
                <button :disabled="count == 200" class="plus" @click="count++">
                  +
                </button>
                <button :disabled="count == 1" class="mins" @click="count--">
                  -
                </button>
              </div>
              <div class="add">
                <a href="javascript:;" @click="addSkuCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
import { regCount } from "@/utils/reg";
import { mapGetters } from "vuex";
import { reqAddOrUpdateCart } from '@/apis';
// import './js/detail';
export default {
  name: "Detail",
  props: ["id"],
  ...
  methods: {
    // 点击那个选中那个（排他思想）
    addActive(currentList, saleAttrValue) {
      currentList.forEach((item) => (item.isChecked = "0"));
